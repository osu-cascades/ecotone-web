class MycorrhizalFungiSamplesController < ApplicationController
  ...
  def index
    @mycorrhizal_fungi_samples = MycorrhizalFungiSample.all
  end
  ...
  private
    ...
    def mycorrhizal_fungi_sample_params
      params.require(:mycorrhizal_fungi_sample).permit(:collected_on, :length, :magnification, :vesicle_count, :arbuscule_count, :visible_hyphae, :hyphae_coverage, :user_id, :plot_id, :plant_id)
    end
